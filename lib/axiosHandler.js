...
const createShortLink = async (data) => {
  return axiosPublicRequest.post(`/links`, data)
    .then((response) => {
...
const axiosShortLink = async (hash) => {
  return axiosPublicRequest.get(`/links/${hash}`)
    .then((response) => {
      return Promise.resolve(response.data)
    }).catch((error) => {
      console.log(error)
 ...
 * @param hash
 */
function getLinkAnalytics(hash) {
  const {
    data,
    error,
    mutate
  } = useSWR(() => {
    // prevents calling undefined instead of hash
    // source: https://bit.ly/2YZ7sUx
    if (hash !== undefined) {
      return [`/links/${hash}/analytics`, hash]
    }
    return null
  }, fetcher)
  return {
...
export {createShortLink, getLinkAnalytics, axiosShortLink}
